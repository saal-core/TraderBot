...
class ComparisonHandler:
    ...
    def _fetch_external_data(
        ...
        try:
            # Use the internet handler to fetch data
            response = self.internet_handler.fetch_data(query_hint, chat_history)
    ...
    def is_comparison_query(self, query: str) -> bool:
        """
        Quick check if a query is likely a comparison query.
        Used for fast routing without full LLM call.
        ...
        comparison_keywords = [
            ...
            'performance against', 'return vs', 'returns vs'
        ]
        ...
        # Check for patterns like "portfolio vs index" or "my stocks against market"
        comparison_patterns = [
            r'portfolio.*(?:vs|versus|against|to).*(?:s&p|nasdaq|dow|index|market|benchmark)',
            r'(?:my|our).*(?:vs|versus|against|compared).*',
            r'(?:compare|comparing).*(?:portfolio|holdings|stocks|returns)',
        ...
        return has_comparison_keyword or has_comparison_pattern